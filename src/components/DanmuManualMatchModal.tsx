...
export default function DanmuManualMatchModal({
  ...
  return createPortal(
    <>
      <div className='fixed inset-0 z-[1008] bg-black/65 backdrop-blur-sm' onClick={onClose} />
      <div className='fixed inset-0 z-[1009] flex flex-col bg-gray-950/95 pb-[env(safe-area-inset-bottom)] pt-[env(safe-area-inset-top)] md:inset-auto md:left-1/2 md:top-1/2 md:max-h-[90vh] md:w-[min(96vw,1080px)] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-2xl md:border md:border-white/20 md:pb-0 md:pt-0 md:shadow-2xl'>
        {/* Header */}
        <div className='flex shrink-0 items-center justify-between border-b border-white/10 px-4 py-3 sm:px-5'>
          <div>
            <h2 className='text-base font-semibold text-white sm:text-lg'>手动匹配弹幕</h2>
            <p className='text-xs text-slate-400'>搜索番剧并指定集数，立即覆盖当前弹幕</p>
          </div>
          <button type='button' onClick={onClose} className='rounded-full p-2 text-slate-300 transition hover:bg-white/10 hover:text-white' aria-label='关闭手动匹配弹窗'>
            <X className='h-5 w-5' />
          </button>
        </div>
        ...
        <div className='min-h-0 flex-1 overflow-y-auto md:grid md:grid-cols-2 md:overflow-hidden'>
          ...
          <div className='border-b border-white/10 md:border-b-0 md:border-r md:border-white/10 md:flex md:flex-col md:min-h-0'>
            ...
            <div className='p-3 sm:p-4 md:flex-1 md:overflow-y-auto'>
              {searching && results.length === 0 ? (
                <div className='rounded-lg border border-white/10 bg-white/5 p-4 text-center text-sm text-slate-300'>正在搜索...</div>
              ) : results.length === 0 ? (
                <div className='rounded-lg border border-dashed border-white/15 bg-white/5 p-4 text-center text-sm text-slate-300'>暂无结果</div>
              ) : (
                  ...
                  {results.map((anime) => (
                    ...
                    >
                      <div className='h-14 w-10 shrink-0 overflow-hidden rounded bg-slate-800/80'>
                        {anime.imageUrl ? (
                          <img src={anime.imageUrl.replace(/^http:\/\//, 'https://')} alt={anime.animeTitle} className='h-full w-full object-cover' onError={(e) => { const t = e.currentTarget; t.style.display = 'none'; if (t.nextElementSibling) (t.nextElementSibling as HTMLElement).style.display = 'flex'; }} />
                        ) : null}
                        <div className='h-full w-full items-center justify-center text-[10px] text-slate-400' style={{ display: anime.imageUrl ? 'none' : 'flex' }}>无封面</div>
                      </div>
                      <div className='min-w-0 flex-1'>
                        <p className='truncate text-sm font-medium text-white'>{anime.animeTitle}</p>
                        <p className='mt-0.5 truncate text-xs text-slate-400'>{buildAnimeMeta(anime)}</p>
                        <p className='mt-0.5 text-[11px] text-slate-500'>共 {anime.episodes.length} 集</p>
                      </div>
                    </button>
                  ))}
                </div>
          ...
          <div className='md:flex md:min-h-0 md:flex-col'>
            <div className='flex items-center justify-between border-b border-white/10 px-4 py-2 text-xs text-slate-400 sm:px-5'>
              <span>{selectedAnime ? `集数列表：${selectedAnime.animeTitle}` : '集数列表'}</span>
              {selectedAnime && selectedAnime.episodes.length > 0 && (
                <div className='flex items-center gap-2'>
                  <button type='button' onClick={() => setEpisodeDescending((prev) => !prev)} className='rounded-md p-1.5 text-slate-400 transition hover:bg-white/10 hover:text-green-400' title={episodeDescending ? '切换正序' : '切换倒序'}>
                  ...
                  <div className='flex items-center gap-1 rounded-md bg-gray-800/80 p-1'>
                    <button type='button' onClick={() => setEpisodeViewMode('list')} title='列表视图' className={`rounded px-1.5 py-1 text-xs font-medium transition-colors ${episodeViewMode === 'list' ? 'bg-green-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>
                    ...
                    <button type='button' onClick={() => setEpisodeViewMode('grid')} title='格子视图' className={`rounded px-1.5 py-1 text-xs font-medium transition-colors ${episodeViewMode === 'grid' ? 'bg-green-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>
                      <svg className='h-3.5 w-3.5' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth='2' d='M4 4h6v6H4V4zm10 0h6v6h-6V4zM4 14h6v6H4v-6zm10 0h6v6h-6v-6z' /></svg>
                    </button>
                  </div>
                </div>
              )}
            ...
            <div className='p-3 sm:p-4 md:flex-1 md:overflow-y-auto'>
              {!selectedAnime ? (
                <div className='rounded-lg border border-dashed border-white/15 bg-white/5 p-4 text-center text-sm text-slate-300'>请先选择左侧番剧</div>
              ) : (
                <>
                        ...
                        return (
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                await onApply({ animeId: selectedAnime.animeId, animeTitle: selectedAnime.animeTitle, episodeId: ep.episodeId, episodeTitle: ep.episodeTitle || `第${originalIndex + 1}集` });
                              } finally {
                                setApplyingEpisodeId(null);
                              }
                            }}
                            className={`flex w-full items-center justify-between gap-3 rounded-lg border px-3 py-2 text-left transition ${
                              currentMark ? 'border-cyan-400/60 bg-cyan-500/10' : 'border-white/10 bg-white/5 hover:bg-white/10'
                            } ${applyingEpisodeId !== null ? 'cursor-not-allowed opacity-70' : ''}`}
                          >
                            <span className='truncate text-sm text-slate-100'>{ep.episodeTitle || `第${originalIndex + 1}集`}</span>
                            <span className='shrink-0 text-xs text-slate-400'>
                              {applying ? (<span className='inline-flex items-center gap-1'><Loader2 className='h-3.5 w-3.5 animate-spin' />应用中</span>) : currentMark ? '当前集' : '使用此集'}
                            </span>
                          </button>
                        );
                      })}
                        ...
                        return (
                          <button
                            ...
                            className={`rounded-lg px-3 py-2 text-sm font-medium transition-all ${
                              currentMark ? 'bg-cyan-500 text-white shadow-md' : 'bg-white/5 text-slate-100 hover:bg-white/10 border border-white/10'
                            } ${applyingEpisodeId !== null ? 'cursor-not-allowed opacity-70' : ''}`}
                            title={ep.episodeTitle || `第${originalIndex + 1}集`}
                          >
                            <div className='truncate'>{applying ? <Loader2 className='h-4 w-4 animate-spin mx-auto' /> : originalIndex + 1}</div>
                          </button>
                        );
                      })}
                    </div>
