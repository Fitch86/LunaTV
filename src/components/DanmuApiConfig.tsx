...
const DanmuApiConfig = ({ config, refreshConfig }: DanmuApiConfigProps) => {
  ...
  const getCurrentApiConfig = () => {
    if (settings.useCustomApi && settings.customApiUrl) {
      return { url: settings.customApiUrl.replace(/\/$/, ''), token: settings.customToken };
    }
    return { url: DEFAULT_DANMU_API_URL, token: DEFAULT_DANMU_API_TOKEN };
  };
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      if (data.success && data.animes && data.animes.length > 0) {
        setTestResult({ success: true, message: `连接成功！找到 ${data.animes.length} 个匹配结果`, count: data.animes.length });
      ...
      if (error instanceof DOMException && error.name === 'AbortError') {
        setTestResult({ success: false, message: `连接超时 (${settings.timeout}秒)` });
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      if (settings.useCustomApi && settings.customApiUrl) {
        try { new URL(settings.customApiUrl); } catch { showMessage('error', '请输入有效的 API 地址'); return; }
      }
  ...
  return (
    <div className='bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 sm:p-6'>
      <div className='flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6'>
        ...
        <h2 className='text-lg sm:text-xl font-bold text-gray-900 dark:text-gray-100'>弹幕API配置</h2>
      </div>

      {message && (
        <div className={`mb-4 p-4 rounded-lg flex items-center gap-2 ${message.type === 'success' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'}`}>
      ...
      <div className='space-y-6'>
        <div className='bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4'>
          <h4 className='text-sm font-semibold text-purple-900 dark:text-purple-300 mb-2'>功能说明</h4>
        ...
        <div className='border border-gray-200 dark:border-gray-700 rounded-lg p-4'>
          <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4'>
            <div className='flex-1 min-w-0'>
              <h3 className='text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100'>启用弹幕功能</h3>
              <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400'>启用后播放器可以加载外部弹幕数据</p>
            </div>
          ...
          {settings.enabled && (
            <div className='space-y-4'>
              <div className='bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3'>
                ...
                <div className='font-mono text-sm text-gray-900 dark:text-gray-100 break-all'>{getCurrentApiConfig().url}</div>
              </div>

              <div className='flex items-center justify-between'>
                <div>
                  <h4 className='text-sm font-medium text-gray-900 dark:text-gray-100'>使用自定义API</h4>
                  <p className='text-xs text-gray-500 dark:text-gray-400'>关闭则使用默认弹幕服务</p>
                </div>
                <label className='relative inline-flex items-center cursor-pointer'>
                  <input type='checkbox' checked={settings.useCustomApi} onChange={(e) => setSettings((prev) => ({ ...prev, useCustomApi: e.target.checked }))} className='sr-only peer' />
                  <div className="w-11 h-6 bg-gray-200 dark:bg-gray-700 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-purple-300 dark:peer-focus:ring-purple-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-purple-600"></div>
                </label>
              </div>
                ...
                <div className='space-y-4 border-t border-gray-200 dark:border-gray-700 pt-4'>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setSettings((prev) => ({ ...prev, customToken: e.target.value }))}
                      placeholder='your-token'
                      className='w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm'
                    />
                    <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>部署弹幕API时设置的 TOKEN 值</p>
                  </div>
                </div>
              )}

              <div>
                <label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1'>请求超时时间 (秒)</label>
                <input
                  type='number'
                  min={5}
                  max={60}
                  ...
                  onChange={(e) => {
                    ...
                    if (val === '') { setSettings((prev) => ({ ...prev, timeout: '' as unknown as number })); return; }
                    const num = parseInt(val);
                    if (!isNaN(num)) setSettings((prev) => ({ ...prev, timeout: num }));
                  }}
                  onBlur={() => {
                    const num = typeof settings.timeout === 'number' && !isNaN(settings.timeout) ? settings.timeout : 30;
                    setSettings((prev) => ({ ...prev, timeout: Math.max(5, Math.min(60, num)) }));
                  }}
                ...
                />
                <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>范围 5-60 秒，建议 30 秒</p>
              </div>
              ...
              <div className='border-t border-gray-200 dark:border-gray-700 pt-4'>
                ...
                {testResult && (
                  <div className={`mt-3 p-3 rounded-lg flex items-center gap-2 ${testResult.success ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'}`}>
                    {testResult.success ? <CheckCircle className='h-4 w-4 flex-shrink-0' /> : <AlertCircle className='h-4 w-4 flex-shrink-0' />}
              ...
              <div className='bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 overflow-hidden'>
                <h4 className='text-sm font-semibold text-blue-900 dark:text-blue-300 mb-2'>默认弹幕服务</h4>
                <div className='text-xs text-blue-800 dark:text-blue-300 space-y-1'>
                  <div className='flex flex-col sm:flex-row sm:items-center gap-1'>
                    <span className='font-medium shrink-0'>API地址：</span>
                    <code className='bg-blue-100 dark:bg-blue-800/50 px-1 rounded break-all'>{DEFAULT_DANMU_API_URL}</code>
                  </div>
                  <div className='flex flex-col sm:flex-row sm:items-center gap-1'>
                    <span className='font-medium shrink-0'>Token：</span>
                    <code className='bg-blue-100 dark:bg-blue-800/50 px-1 rounded break-all'>{DEFAULT_DANMU_API_TOKEN}</code>
                  </div>
                </div>
              </div>
            </div>
